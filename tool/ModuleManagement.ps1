...
if ($UninstallModuleName -ne "") {
    ...
    foreach ($Module in $ModuleList) {
        if (($Module).RawAttributes.Count -ne 0 -and ($Module).RawAttributes.name.CompareTo($UninstallModuleName) -eq 0) {
            ...
            $command = $env:SystemRoot + "\System32\inetsrv\appcmd uninstall module " + $UninstallModuleName+" >null"
...
if ($InstallModuleName -ne "" -and $ModulePath -ne "") {
    $command = $env:SystemRoot + "\System32\inetsrv\appcmd install module /name:" + $InstallModuleName + " /image:" + $ModulePath+" >null"
    Invoke-Expression $command
    Write-Output("[ps] Module Installed.")
}else{
    Write-Output("[ps] Parameter number not enough.")
}